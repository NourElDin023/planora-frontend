  ...
  const fetchCollections = async () => {
    try {
      ...
      if (res.data.length > 0) {
        handleCollectionClick(res.data[0]); // auto-select the first collection
      }
    } catch (err) {
  ...
  {collections.map((collection) => (
    <li
      ...
      style={{
        ...
        borderRadius: "6px",
        backgroundColor:
          selectedCollection?.id === collection.id ? "rgba(125, 38, 205, 0.1)" : "transparent",
        border:
          selectedCollection?.id === collection.id ? "1px solid #7D26CD" : "none",
          ...
          selectedCollection ? (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <TaskManager collectionId={selectedCollection.id} />
              </div>
            </div>
          ) : (
